...
def build_parser():
    ...
    parser.add_argument("--base-url",
                        default=None,
                        metavar="URL",
                        help="API base URL for this run only (overrides the "
                             "`base_url` field in config.json), to try another "
                             "LLM provider without editing the config")
    parser.add_argument("--api-key",
                        default=None,
                        metavar="KEY",
                        help="API key for this run only (overrides "
                             "~/.config/cai/api_key, which then need not exist)")
...
def main(argv=None):
    ...
    # --base-url / --api-key: registered before the first load_config /
    # load_api_key call so every consumer in the process - this bootstrap, the
    # TUI's models registry, sub-agents falling back to config - sees them.
    config.set_override("base_url", args.base_url)
    config.set_override("api_key", args.api_key)
