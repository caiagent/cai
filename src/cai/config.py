...
Above everything sit runtime overrides (set_override), registered by the CLI's
--base-url / --api-key flags for one-off runs against another provider."""
import os
import sys
import json
import logging
...
# runtime overrides registered by the CLI (--base-url / --api-key): the
# highest-precedence layer, above both the cai.settings shadow and the files.
# lets a single run target another provider without editing config.json or
# the api_key file.
_overrides = {}


def set_override(key, value):
    """register a runtime override for a config field or 'api_key'. a None
    value is ignored, so CLI args can be passed through unconditionally."""
    if value is None:
        return
...
def load_api_key():
    ...
    telling the user how to create it. an --api-key override wins outright -
    the file does not need to exist."""
    override = _overrides.get("api_key")
