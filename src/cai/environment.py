...
function tools registered via cai.tool, the skill-body slot fillers registered
via cai.slot, the MCP servers declared via cai.mcp_server, the hooks and
commands registered via cai.hook / cai.command, the discovered extension
bundles, and the live Settings (cai.settings). An
...
class Environment:
    ...
    def __init__(self, extensions=None):
        ...
        self._slots = {}             # exposed name -> callable (skill-body slots)
    ...
    def register_slot(self, fn):
        """record a skill-body slot filler (cai.slot's backing), keyed by its
        exposed name. namespaced '<extension>__<name>' while an extension loads,
        exactly like register_tool, so a skill body references it as
        '{{<extension>__<name>}}'. a later registration of the same name wins
        (the user's init runs last)."""
        name = fn.__name__
        extension = self._loading_extension
        if extension and extension != "user":
            name = f"{extension}__{name}"
        fn._cai_slot_name = name
        self._slots[name] = fn

    def slot(self, name):
        """the slot filler registered under `name` via cai.slot, or None."""
        return self._slots.get(name)

    def slots(self):
        """the registered slot fillers as exposed name -> callable (a copy)."""
