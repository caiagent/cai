...
class ToolsRegistry:
    ...
    dispatch itself is the programmatic API and does not check selection -
    dispatch_selected is the model-facing variant that refuses unselected names
    (the Agent run loop and the python tool's tool_call() both route through
    it), so a model naming an unselected tool is refused even though the
    registration would run it.
    ...
    def dispatch_selected(self, name, arguments):
        """run one tool by its exposed name, refusing any that isn't selected -
        the model-facing dispatcher. registered-but-unselected tools (wrap
        targets, deselected tools, the always-registered sub-agent tools) stay
        reachable through dispatch programmatically, but a model that names one
        gets the same error as for a tool that doesn't exist. checks selection
        per call, so tools a skill selects mid-run become callable
        immediately."""
...
def wrap(target):
    """decorator: register a function tool that wraps another tool, e.g.
        ...
        @cai.wrap("fs__read_file")
        def read_note(call, id: str):
            \"\"\"Read a note from cai's memory vault.\"\"\"
            return call(file_path=f"~/.config/cai/notes/{id}.md")
